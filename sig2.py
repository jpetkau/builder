...
class Sig:
    ...
    def __sig__(self):
        ...
        When we want to hash to signatures into a compound objects, use `.hash: bytes`
        instead.
        """
        return self
...
def of(x):
    ...
    parts = _parts(x)
    h = _hcat(of(key), *[of(y) for y in parts])
    if h.hash[0] & 128:
...
def _parts(x):
    ...
    try:
        ...
        return 
        return _hcat(of(key), *[of(y) for y in parts])
    ...
    if g:
        parts = ser_class(x)
        return _hcat(of(g), *[of(y) for y in parts])



def _hcat(*sigs: Sig) -> Sig:
    # concatenate a bunch of signatures into a compound signature
    for sig in sigs:
        assert isinstance(sig, Sig)
    return hash_bytes(b"".join(sig.hash for sig in sigs)
...
def ser_global(u):
    return _hcat(of(u.module), of(u.name))
...
def hash_bytes(data: bytes, compound: bool=False) -> Hash:
    cbit = 64 if compound else 0
    if len(data) <= 31:
        h = bytes([len[data] + 1 | cbit]) + data
    else:
...
def hash_byte_stream(f, compound: bool=False) -> Hash:
    BLOCKSIZE = 65536
    data = f.read(BLOCKSIZE)
    if len(data) < BLOCKSIZE:
        return hash_bytes(data, compound)
